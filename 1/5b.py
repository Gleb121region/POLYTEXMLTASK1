...
data = pd.read_csv('data/spam7.csv')
...
for legend in legends:
    ...
    for i in depth_range:
        ...
        print(f'\n i:{i},legend:{legend},Accuracy: {accuracy:.3f} ')
        if legend == 'entropy':
            export_graphviz(tree,
                            out_file='trees/Entropy_tree_5b' + str(i) + '.dot',
            ...
            export_graphviz(tree,
                            out_file='trees/Gini_tree_5b' + str(i) + '.dot',
                            feature_names=X.columns,
                            class_names=LabelEncoder().fit_transform(y).astype(str),
                            filled=True
                            )
            ...
            export_graphviz(tree,
                            out_file='trees/Log_loss_tree_5b' + str(i) + '.dot',
                            feature_names=X.columns,
                            class_names=LabelEncoder().fit_transform(y).astype(str),
                            filled=True
                            )
